...
import (
	...
	V1Handlers "github.com/dmzsz/duozhuayu/internal/http/handlers/v1"
...
func setup(t *testing.T) {
	...
	// selectData := rolesDataFromDB[1:1]
	usersDataFromDB = []V1Domains.UserDomain{
		{
...
func lazyAuth(ctx *gin.Context) {
	...
	var roleIds []string
	for _, role := range *userDataFromDB.Roles {
	...
	jwtClaims := jwt.JWTClaims{
		...
		RegisteredClaims: GolangJWT.RegisteredClaims{
			ExpiresAt: GolangJWT.NewNumericDate(time.Now().Add(time.Hour * time.Duration(configs.AppConfig.SecurityConfig.JWT.AccessKeyTTL))),
