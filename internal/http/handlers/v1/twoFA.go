...
import (
	...
	V1Domains "github.com/dmzsz/duozhuayu/internal/domain/v1"
	"github.com/dmzsz/duozhuayu/pkg/helpers"

	// "github.com/dmzsz/duozhuayu/lib/middleware"
	"github.com/dmzsz/duozhuayu/service"
)

// Setup2FA handles jobs for controller.Setup2FA
func Setup2FA(claims JWTClaims, user V1Domains.UserDomain) (httpResponse model.HTTPResponse, httpStatusCode int) {
	// check auth validity
	...
	// 2FA already enabled
	configSecurity := configs.AppConfig.SecurityConfig
	...
	verifyPass, err := argon2.ComparePasswordAndHash(user.Password, configSecurity.HashSec, v.Password)
	if err != nil {
	...
	// get user email
	if !helpers.ValidateEmail(v.Email) {
	...
	img, err := helpers.ByteToPNG(qrByte, configSecurity.TwoFA.PathQR+"/")
	if err != nil {
	...
	hashPass, err := service.GetHash([]byte(user.Password))
	if err != nil {
	...
	if ok {
		...
		if helpers.FileExist(configSecurity.TwoFA.PathQR + "/" + data2FA.Image) {
...
func Activate2FA(claims middleware.MyCustomClaims, authPayload model.AuthPayload) (httpResponse model.HTTPResponse, httpStatusCode int) {
	...
	configSecurity := configs.GetConfig().Security
	if claims.TwoFA == configSecurity.TwoFA.Status.Verified {
		// JWT: 2FA verified, abort setup
		httpResponse.Message = "twoFA: " + configSecurity.TwoFA.Status.Verified
		httpStatusCode = http.StatusBadRequest
	...
	authPayload.OTP = helpers.RemoveAllSpace(authPayload.OTP)
	...
	keyMainCipherByte, err := helpers.Encrypt(otpByte, data2FA.PassSHA)
	if err != nil {
	...
	keyBackupCipherByte, err := helpers.Encrypt(otpByte, keyRecoveryHash)
	if err != nil {
	...
	uuidEncByte, err := helpers.Encrypt(uuidPlaintextByte, keyRecoveryHash)
	if err != nil {
...
func Validate2FA(claims middleware.MyCustomClaims, authPayload model.AuthPayload) (httpResponse model.HTTPResponse, httpStatusCode int) {
	...
	if newProcess {
		...
		keyMainPlaintextByte, err := helpers.Decrypt(keyMainCipherByte, data2FA.PassSHA)
		if err != nil {
