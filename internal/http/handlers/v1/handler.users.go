...
func (userH UserHandler) SendOTP(ctx *gin.Context) {
	...
	go userH.redisCache.Set(otpKey, otpCode, 0)
...
func (c UserHandler) GetUserData(ctx *gin.Context) {
	...
	userClaims := ctx.MustGet(constants.CtxAuthenticatedUserKey).(jwt.JWTClaims)
	...
	userDom, statusCode, err := c.usecase.GetByEmail(ctxx, &V1Domains.UserDomain{Email: userClaims.Email}, false)
	if err != nil {
		NewErrorResponse(ctx, statusCode, err.Error())
		return
	}
