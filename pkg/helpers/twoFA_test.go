...
func TestTOTP(t *testing.T) {
	for _, test := range tests {
		...
		otpByte, err := helpers.NewTOTP(test.account, test.issuer, test.hash, test.digits)
		if err != nil {
		...
		qrByte, err := helpers.NewQR(otpByte, test.issuer)
		if err != nil {
		...
		_, err = helpers.ValidateTOTP(encryptedMessage, test.issuer, validToken)
		if err != nil {
			t.Fatalf("failed to validate a valid TOTP token: %v", err)
		}
		// test with an invalid token
		invalidToken := createInvalidToken(validToken)
		_, err = helpers.ValidateTOTP(encryptedMessage, test.issuer, invalidToken)
		if err == nil {
